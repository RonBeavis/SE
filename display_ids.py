...
def tsv_file(_ids,_scores,_spectra,_kernel,_job_stats,_params):
	...
	if not ofile:
		...
		return False
	valid_ids = 0
	line = 'PSM\tspectrum\tprotein\tstart\tend\tsequence\tmodifications\tions\tscore\tdM\n'
	ofile.write(line + '\n')
	psm = 1
	for j in _ids:
		if len(_ids[j]) == 0:
				...
				for k in kern['mods']:
					for c in k:
						if k[c] in modifications:
							...
							line += '%s%s#%.3f;' % (lseq[int(c)-int(kern['beg'])],c,k[c]/1000)
				line += '\t'
				line += '%i\t%i\t%.3f\n' % (_scores[j],100.0*_scores[j]/float(len(kern['seq'])),(_spectra[j]['pm']-(kern['pm']))/1000.0)
		ofile.write(line)
	ofile.close()
	print('\n3. Output parameters:')
